				...
				if clicked:
					...
					if _last_clicked_player == clicked and time_since_last < DOUBLE_CLICK_TIME:
						...
						if clicked.is_handle_extended():
							...
							print("⏸️ Луч выключен (ОЖИДАНИЕ)")
						else:
							clicked.enable_ray()
							clicked.start_editing()
							print("🔫 Луч включен, ручка для изменения длины активна")
